...
public class UserControls extends View {
	...
	private HBox makeBackgroundPickerBox() {
		...
		return makePickerBox(this.getController().getValueReader().getValue("BackgroundLabel"), picker);
	}
	
	private HBox makePenPickerBox() {
		ColorPicker picker = makePenPicker();
		return makePickerBox(this.getController().getValueReader().getValue("PenLabel"), picker);
	}
	...
	private Button makeChangeImageButton() {
		Button btn = new Button(this.getController().getValueReader().getValue("TurtleImageSelector"));
		btn.setOnAction(e -> {
			FileChooser fileChooser = new FileChooser();
			fileChooser.setTitle(this.getController().getValueReader().getValue("TurtleImageSelector"));
